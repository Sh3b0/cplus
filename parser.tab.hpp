...
switch (yytype)
    ...
    struct symbol_type : basic_symbol<by_type>
    {
      ...
      {
        YY_ASSERT (tok == token::END || tok == token::VAR || tok == token::IS || tok == token::INT || tok == token::REAL || tok == token::BOOLEAN);
      }
#else
      symbol_type (int tok)
        : super_type(token_type (tok))
    ...
    /// Build a parser object.
     parser  (cplus::lexer &lexer_yyarg);
    virtual ~ parser  ();
